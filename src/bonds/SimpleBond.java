...
/**
 * SimpleBond 클래스는 채권 이자 지급 방법 중 단리채 계산을 제공하는 클래스입니다.
 */
public class SimpleBond implements Bond {
     ...
     */
    public SimpleBond(int purchasePrice, int interestCalculationPeriod, double couponRate, int faceValue, Date maturityDate, Date purchaseDate, int investmentAmount) {
    ...
    public void calculateSimpleInterestAmount() {
        ...
        System.out.println("단리 이자: " + simpleInterest);
    }

    /**
     * 세전 원금액 계산 함수
     *
     * 단리채의 세전 원금액을 계산하고 출력합니다.
     */
    @Override
    public void calculatePreTaxPrincipalAmount() {
        double preTaxPrincipalAmount = purchasePrice - (faceValue * couponRate * interestCalculationPeriod) / 100;
    ...
    public void calculateYield() {
        double yield = ((faceValue - purchasePrice) / (purchasePrice + (faceValue * couponRate * interestCalculationPeriod) / 100)) * 100;
    ...
    public void calculateYearInterestPayment() {
        double annualInterestPayment = (faceValue * couponRate * interestCalculationPeriod) / 100;
